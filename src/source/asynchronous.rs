...
impl Client {
    ...
    async fn read_responses(&mut self, id: i32) -> io::Result<Vec<Packet>> {
        ...
        while let Ok(response) = self.read_packet(id).await {
    ...
    async fn read_packet(&mut self, id: i32) -> io::Result<Packet> {
        let packet = Packet::read_from_async(&mut self.tcp_stream).await?;
...
impl AsyncRCon for Client {
    async fn login(&mut self, password: &str) -> io::Result<bool> {
        ...
        loop {
            ...
            packet = Packet::read_from_async(&mut self.tcp_stream).await?;
